...
def entrenar_modelo_gam(df, fecha_inicio, fecha_fin):
    df_train = df[(df['Fecha'] >= pd.to_datetime(fecha_inicio)) & (df['Fecha'] <= pd.to_datetime(fecha_fin))].copy()
    df_train = df_train.dropna(subset=['Long', 'Lat', 'Fecha'])

    min_fecha = df_train['Fecha'].min()
    df_train['t'] = (df_train['Fecha'] - min_fecha).dt.total_seconds() / 86400.0  # días con decimales
...
def simular_eventos(df, fecha_inicio_train, fecha_fin_train,
    ...
    t_ini = 0
    t_fin = (pd.to_datetime(fecha_fin_sim) - pd.to_datetime(fecha_inicio_sim)).total_seconds() / 86400.0
